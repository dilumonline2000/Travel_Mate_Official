...
const BookingConfirmation = () => {
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        ...
        console.log("asdasd  ",activityData)
  
        // Generate QR code URL
        const qrCodeData = `Activity: ${activityData.activityName}, Date: ${activityData.date}, Location: ${activityData.activityLocation}, Price: ${activityData.price}`;
  ...
  if (!activity) {
    return <Typography variant="h6" align="center">Loading booking...</Typography>;
  }
          ...
          <Card variant="outlined">
            <CardContent>
              ...
              <Typography variant="h6" align="center">
                Activity: {activity?.activityName || "No data available"}
              </Typography>
              <Typography variant="body1" align="center">
                Date: {activity?.date || "No data available"}
              </Typography>
              <Typography variant="body1" align="center">
                Location: {activity?.activityLocation || "No data available"}
              </Typography>
              <Typography variant="body1" align="center">
                Price: {activity?.price || "No data available"}
              </Typography>
              {qrCodeUrl && (
